...
const Button = ({
  ...
  const styles = {
    ...
    error: `${base_style} text-white rounded-full bg-error w-full `,
    cancel: `${base_style} text-natural-light rounded-full bg-white w-full border  border-natural-light `,
  };
  if (to)
    return (
  ...
  return (
    ...
    >
      {children}
      {loading && <Spinner className={`w-[18px] h-[18px] ${loaderClass} `} />}
    </button>
  );
};
